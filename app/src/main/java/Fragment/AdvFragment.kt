...
class AdvFragment : Fragment() {
     ...
     fun getCats() {
          ...
          var call : Call<List<Cat>> =
                    apiClient.getClient().create(ApiService::class.java).getAllCat()
     ...
     fun addRecord() {
          var apiClient = ApiClient()
          var call : Call<myResult> =
                    apiClient.getClient().create(ApiService::class.java).addRecord(
          ...
          call.enqueue(object : Callback<myResult> {
               override fun onResponse(call : Call<myResult>? , response : Response<myResult>?) {
                    ...
                    if (res !!.result.equals("ok")) {
                         Toast.makeText(context , "اگهی با موفقیت ثبت شد" , Toast.LENGTH_LONG)
                                   .show()
                    }
                    else {
